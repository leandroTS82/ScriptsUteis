...
====================================================================================
 ...
 Funções:
   ...
   - Preview colorido
   - Registro em BaseTerms.json e TranscriptResults.json
...
CREATE_LATER = f"C:\\Users\\leand\\LTS - CONSULTORIA E DESENVOLVtIMENTO DE SISTEMAS\\EKF - English Knowledge Framework - Base\\BaseTerms\\BaseTerms_{current_date}.json"
FULL_RESULTS = r"C:\Users\leand\LTS - CONSULTORIA E DESENVOLVtIMENTO DE SISTEMAS\EKF - English Knowledge Framework - Books\TranscriptResults.json"

BASE_TERMS_DIR = r"C:\Users\leand\LTS - CONSULTORIA E DESENVOLVtIMENTO DE SISTEMAS\EKF - English Knowledge Framework - Base\BaseTerms"
...
def load_external_terms() -> list[str]:
    if not os.path.exists(BASE_TERMS_DIR):
        return []

    consolidated = set()

    for filename in os.listdir(BASE_TERMS_DIR):
        if not filename.endswith(".json"):
            continue

        path = os.path.join(BASE_TERMS_DIR, filename)

        try:
            with open(path, "r", encoding="utf-8") as f:
                data = json.load(f)

            candidates = data.get("terms") or data.get("pending") or []
            if isinstance(candidates, list):
                for term in candidates:
                    if isinstance(term, str) and term.strip():
                        consolidated.add(term.strip().lower())
...
def save_create_later(item: str):
    ...
    if not os.path.exists(CREATE_LATER):
        ...
        print(f"📌 BaseTerms.json criado → {item}")
        return
