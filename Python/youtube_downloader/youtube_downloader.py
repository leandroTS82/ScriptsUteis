...
def download_youtube(
    ...
    if download_subs:
        command += [
            ...
            "--write-auto-subs",
            "--sub-langs",
            f"{PRIMARY_SUB_LANG},{FALLBACK_SUB_LANG}",
            "--sub-format",
            "srt",
            "--convert-subs",
            "srt",
...
if __name__ == "__main__":
    ...
    if partition:
        try:
            t_ini = parse_time_input(
                input("t_ini (segundos ou HH:MM:SS): ")
            )
            t_end = parse_time_input(
                input("t_end (segundos ou HH:MM:SS): ")
            )
    ...
    name_input = input(
        "Nome do arquivo (ou 'n' para usar o padrão): "
    ).strip()

    custom_name = None if name_input.lower() == "n" else name_input

    download_subs = (
        input("Deseja baixar a legenda? (s/n): ")
        .strip()
        .lower() == "s"
    )
